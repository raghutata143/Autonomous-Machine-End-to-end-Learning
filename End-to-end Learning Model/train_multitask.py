...
def train_model(model, args, X_train, X_valid, y_train, y_valid,data_type="unity3d"):
    checkpoint = ModelCheckpoint('model-best-multitask2-speed-normalized_trial3.h5', monitor='val_loss', verbose=1, save_best_only=args['save_best_only'], mode='auto')
    model.compile(loss='mean_squared_error', optimizer=Adam(lr=args["learning_rate"]))
    csv_logger = CSVLogger('nvidia_speed_normalized_multitask2_50_epoch_trial3.log', separator = ',', append = True)
