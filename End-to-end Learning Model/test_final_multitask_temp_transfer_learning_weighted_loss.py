...
#    image = preprocess(args,image)
    ...
    print(diff[-1])
#    print(pred[0][0][0])
np.savetxt("situaion_from_scratch_with_dropouts_transfer_learning_weighted_loss.txt",diff, delimiter=",")
prediction =  pd.DataFrame(prediction, columns = ["angle","speed"])
prediction.to_csv("situation_from_scratch_with_dropouts_transfer_learning_weighted_loss.csv", index = False)
