...
def load_udacity_normalized1(number):
    path = os.path.abspath(os.getcwd()+"/../data/udacity-"+str(number))
    data_df = pd.read_csv(path+"/interpolated.csv")
    data_df["left"] = data_df["filename"].apply(lambda x:path+"/"+x if x.split("/")[0]=="left" else None)
    data_df["center"] = data_df["filename"].apply(lambda x:path+"/"+x if x.split("/")[0]=="center" else None)
    data_df["right"] = data_df["filename"].apply(lambda x:path+"/"+x if x.split("/")[0]=="right" else None)
    data_df = data_df.rename(columns={"angle":"steering_angle"})
    steer = data_df["steering_angle"]
    speed = data_df["speed"]
    steer_mean = np.average(steer)
    steer_diff = np.max(steer) - np.min(steer)
    steer = steer-steer_mean
    steer = steer/steer_diff
    data_df["steering_angle"] = steer
    speed_mean = np.average(speed)
    speed_diff = np.max(speed) - np.min(speed)
    speed = speed-speed_mean
...
def load_data_center(args):
    if(args["data_type"] == "udacity"):
        data_df = load_udacity_normalized(args["data_number"])
...
def train_model(model, args, X_train, X_valid, y_train, y_valid,data_type="unity3d"):
    checkpoint = ModelCheckpoint('model-best.h5', monitor='val_loss', verbose=1, save_best_only=args['save_best_only'], mode='auto')
    ...
    history_callback = model.fit_generator(batch_generator(args,args["data_dir"], X_train, y_train, args["batch_size"], True,data_type=data_type),
                        ...
                        verbose=1)
    steering_angle_loss = history_callback.history["steering_angle_loss"]
    speed_loss = history_callback.history["speed_loss"]
#    loss_history = np.array(loss_history)
    steering_angle_loss = np.array(steering_angle_loss)
    speed_loss = np.array(speed_loss)
    np.savetxt(args["data_type"]+str(args["data_number"])+"/"+"steering_angle_history.txt", steering_angle_loss, delimiter=",")
    np.savetxt(args["data_type"]+str(args["data_number"])+"/"+"speed_loss.txt",speed_loss,delimiter=",")
