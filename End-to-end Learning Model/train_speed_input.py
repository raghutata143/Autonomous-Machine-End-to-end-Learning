...
def load_data_center(args):
    ...
    X = df[['center','speed']].values
    y = df[args["target_variable"]].values            
    print(df.head())
    X_train, X_valid, y_train, y_valid = train_test_split(X, y, test_size=args["test_size"], random_state=0)
    return X_train, X_valid, y_train, y_valid

def load_data_center_normalized2(args):
    if(args["data_type"] == "udacity"):
        data_df = load_udacity_normalized2(args["data_number"])
...
#            if(index == 0):
            speed_temp = image_paths[index][1]
            ...
            if i == batch_size:
                break
        yield {'images':images_batch,'speed':speed}, [target_batch0]


def train_model(model, args, X_train, X_valid, y_train, y_valid,data_type="unity3d"):
    checkpoint = ModelCheckpoint('model-best-speed-input-trial1.h5', monitor='val_loss', verbose=1, save_best_only=args['save_best_only'], mode='auto')
    model.compile(loss='mean_squared_error', optimizer=Adam(lr=args["learning_rate"]))
    csv_logger = CSVLogger('nvidia_speed-input-trial1_50epoch.log', separator = ',', append = True)
...
#model = build_model_nvidia_multi_task(args)
model = build_model_nvidia_with_speed(args)
