...
def train_model(model, args, X_train, X_valid, y_train, y_valid,data_type="unity3d"):
    checkpoint = ModelCheckpoint('model-best-final_multitask_normalized_weighted_loss.h5', monitor='val_steering_angle_loss', verbose=1, save_best_only=args['save_best_only'], mode='auto')
    model.compile(loss=customLoss, optimizer=Adam(lr=args["learning_rate"]))
    csv_logger = CSVLogger('model-best-final_multitask_normalized_weighted_loss.log', separator = ',', append = True)
